...
本项目的核心目标是实践并验证 `spec-to-delivery-workflow` 这个 skill，覆盖从需求到交付的全自动开发流程，包括：

- 需求拆解与阶段化执行（spec）
- 设计与实现联动（implement）
- 测试与验收闭环（testing / checkpoint）
- 进度追踪与持续推进（progress-tracker）

在这个过程中，`gkill` 作为可运行产物，用于承载并验证自动化工作流产出的真实代码与发布流程。
