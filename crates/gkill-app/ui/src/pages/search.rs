...
pub fn SearchPage() -> View {
    ...
    let do_search = move || {
        ...
        spawn_local(async move {
            match api::search_skills(&q, "relevance", pg, 12).await {
        ...
        div(class="flex flex-col h-full") {
            ...
            div(class="flex gap-2.5 px-6 py-4 bg-white border-b border-[#e4e7ef]") {
                ...
                button(
                    ...
                    on:click=move |_| { page_no.set(0); do_search(); }
                ) { "搜索" }
            }
            ...
            (if loading.get() {
                view! {
                    div(class="flex items-center justify-center py-16 text-[#94a3b8] text-sm gap-2") {
                        span(class="animate-spin") { "⟳" }
                        span { "正在加载…" }
                    }
                }
            } else if let Some(e) = error.get_clone() {
                view! {
            ...
            div(class="flex-1 overflow-y-auto px-6 py-5") {
                (if let Some(res) = result.get_clone() {
                    ...
                    view! {
                        // Result count
                        div(class="text-xs text-[#94a3b8] mb-4") {
                            "共 " (total) " 个结果"
                        }
                        div(class="grid grid-cols-1 gap-3") {
                            Indexed(
                                ...
                                view=move |item: SkillItem| {
                                    ...
                                    let ns_btn    = ns.clone();
                                    view! {
                                        div(class="card p-4 flex items-start justify-between gap-4 hover:shadow-md transition-shadow") {
                ...
                view! {
                    div(class="flex items-center justify-center gap-4 px-6 py-3 bg-white border-t border-[#e4e7ef] text-sm") {
                        ...
                        button(
                            ...
                            on:click=move |_| {
                                page_no.set(page_no.get() + 1); do_search();
                            }
                        ) { "下一页 →" }
                    }
                }
